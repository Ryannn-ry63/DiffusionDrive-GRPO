...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 audit|formal public|multi EXPERIMENT_NAME GPU"
  exit 2
fi
PHASE="$1"
BRANCH="$2"
...
[[ "$EXPERIMENT_NAME" =~ ^stage27_selector_(public|multi)_stage24_(audit|formal)_[A-Za-z0-9_.-]+$ ]] || {
...
for path in "$PUBLIC" "$TRAINING_CACHE" "$MANIFEST" "$AUDIT" "${BANKS[@]}"; do
...
def sha(path):
    ...
    return h.hexdigest()
branch,audit_sha,public,manifest,audit,*banks=sys.argv[1:]
...
' "$BRANCH" "$AUDIT_SHA" "$PUBLIC" "$MANIFEST" "$AUDIT" "${BANKS[@]}"
