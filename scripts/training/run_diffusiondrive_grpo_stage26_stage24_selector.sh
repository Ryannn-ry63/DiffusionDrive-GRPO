...
[[ "$EXPERIMENT_NAME" =~ ^stage26_stage24_selector_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid experiment"; exit 2;
}
[[ "$CUDA_DEVICE" =~ ^[0-7]$ ]] || { echo "invalid CUDA device"; exit 2; }
for path in "$BASE_CHECKPOINT" "$MANIFEST" "$AUDIT" "${BANKS[@]}"; do
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON_BIN" navsim/planning/script/run_training.py \
  ...
  seed=26024 dataloader.params.batch_size=2 dataloader.params.num_workers=4 \
  ...
  agent.lr=1e-4 agent.config.grpo_training_mode=stage24_selector \
