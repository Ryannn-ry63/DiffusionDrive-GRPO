#!/usr/bin/env python3
"""Freeze exact Stage34 audit/formal checkpoints and run artifacts."""
...
def choose(records: list[dict], step: int) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage34 checkpoint global_step={step} is missing")
    preferred = [
        record
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(
            f"expected one Stage34 Lightning version under "
    ...
    if [record["epoch"] for record in selected] != expected_epochs:
        raise RuntimeError(
            "Stage34 checkpoint epoch/optimizer-step schedule drifted: "
            f"{[(r['epoch'], r['global_step']) for r in selected]}"
        )
    events = sorted(version.glob("events.out.tfevents.*"))
    if len(events) != 1:
        raise RuntimeError("Stage34 expected exactly one TensorBoard event file")
    override_candidates = (
    ...
    if overrides is None:
        raise RuntimeError("Stage34 Hydra overrides are missing")
