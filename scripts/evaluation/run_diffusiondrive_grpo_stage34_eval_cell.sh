...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 FOLD(0|1) SYSTEM(P|DPEL192|MAF48|MAF96|MAF144|MAF192) NOISE GPU"
  exit 2
fi
...
STAGE34_FREEZE="$ROOT_DIR/artifacts/grpo_stage34/pilot/training/MAF/fold${FOLD}/formal/checkpoints.json"
OUT_DIR="$ROOT_DIR/artifacts/grpo_stage34/pilot/eval/fold${FOLD}"

[[ "$FOLD" =~ ^[01]$ ]] || { echo "fold must be 0 or 1"; exit 2; }
[[ "$SYSTEM" =~ ^(P|DPEL192|MAF48|MAF96|MAF144|MAF192)$ ]] || {
  echo "invalid Stage34 evaluation system: $SYSTEM"; exit 2;
}
[[ "$NOISE" == 20261511 || "$NOISE" == 20261512 ]] || {
  echo "Stage34 noise must be 20261511 or 20261512"; exit 2;
}
[[ "$GPU" =~ ^[0-7]$ ]] || { echo "GPU must be 0..7"; exit 2; }
for path in "$PYTHON_BIN" "$PUBLIC" "$SELECTOR" "$CALIBRATION" "$CV_FREEZE"; do
  [[ -f "$path" ]] || { echo "missing Stage34 eval input: $path"; exit 2; }
done
if [[ "$SYSTEM" == DPEL192 ]]; then
  [[ -f "$DPEL_FREEZE" ]] || { echo "missing DPEL freeze: $DPEL_FREEZE"; exit 2; }
elif [[ "$SYSTEM" == MAF* ]]; then
  [[ -f "$STAGE34_FREEZE" ]] || {
    echo "missing Stage34 formal freeze: $STAGE34_FREEZE"; exit 2;
  }
fi

readarray -t LOCKED < <(
  "$PYTHON_BIN" - \
    "$CV_FREEZE" "$FOLD" "$SYSTEM" "$PUBLIC" "$DPEL_FREEZE" "$STAGE34_FREEZE" <<'PY'
import hashlib
import json
import sys
...
public, dpel_path, stage34_path = map(Path, sys.argv[4:7])
...
if (
    ...
    raise RuntimeError("Stage34 holdout manifest drifted")
if system == "P":
    checkpoint = public
elif system == "DPEL192":
    ...
    if len(matches) != 1 or sha(Path(matches[0]["path"])) != matches[0]["sha256"]:
        raise RuntimeError("Stage34 DPEL192 positive-control freeze drifted")
    checkpoint = Path(matches[0]["path"])
else:
    step = int(system.removeprefix("MAF"))
    freeze = json.loads(stage34_path.read_text())
    matches = [
        item for item in freeze["checkpoints"]
...
[[ "${#LOCKED[@]}" -eq 5 ]] || {
  echo "failed Stage34 evaluation input resolution"; exit 2;
}
GENERATOR="${LOCKED[0]}"
...
DOMAIN="stage34_pilot_${SYSTEM,,}_fold${FOLD}"
...
if [[ "${GRPO_STAGE34_EVAL_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  ...
  echo "PASS Stage34 eval preflight fold=$FOLD system=$SYSTEM noise=$NOISE"
  exit 0
fi
...
if bad:
    raise RuntimeError(f"Stage34 PDM artifact failed provenance checks: {bad}")
print(
    f"PASS Stage34 eval domain={domain} noise={noise} "
