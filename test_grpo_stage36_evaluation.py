...
from scripts.evaluation.summarize_grpo_stage36_pilot import (
    expected_checkpoint,
    summarize,
    trimmed10_mean,
)
...
NCD_EVAL_ROOT = Path("artifacts/grpo_stage35/pilot/eval")


def test_stage36_reuses_exact_paired_public_and_dpel_controls():
    ...
    ncd_sha, ncd_domain = expected_checkpoint(
        NCD_EVAL_ROOT, "NCD192", 0
    )
    assert len(ncd_sha) == 64
    assert ncd_domain == "stage35_pilot_ncd192_fold0"


def test_stage36_summary_preserves_fold_namespace_and_tail_guards():
    count = 40
...
def test_stage36_trimmed_mean_discards_symmetric_tails():
