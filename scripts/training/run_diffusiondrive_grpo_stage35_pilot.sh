...
if [[ $# -lt 4 || $# -gt 5 ]]; then
  echo "Usage: $0 audit|formal|chain NCD HOLDOUT(0|1) EXPERIMENT [CUDA_DEVICES]"
  exit 2
fi
...
PUBLIC="${STAGE35_PUBLIC_CHECKPOINT:-/inspire/hdd/global_user/wangcaojun-240208020180/nry/diffusiondrive_navsim_88p1_PDMS}"
SELECTOR="${STAGE35_SELECTOR_CHECKPOINT:-/inspire/hdd/global_user/wangcaojun-240208020180/nry/exp/stage27_selector_multi_stage25_formal_seed27125/2026.07.24.09.28.29/lightning_logs/version_0/checkpoints/grpo-17-36684.ckpt}"
CALIBRATION="${STAGE35_SELECTOR_CALIBRATION:-$ROOT_DIR/artifacts/grpo_stage27/selectors/multi/calibration.json}"
CV_FREEZE="${STAGE35_CV_FREEZE:-$ROOT_DIR/artifacts/grpo_stage30/manifests/cv/freeze.json}"
DPEL_FREEZE="$ROOT_DIR/artifacts/grpo_stage32/pilot/training/DPEL/fold${HOLDOUT}/formal/checkpoints.json"
PLAN="$ROOT_DIR/GRPO_STAGE35_NESTED_COUNTERFACTUAL_DEPLOYMENT_PLAN_20260727.md"
PLAN_SHA="301e5fb37066c34f6a3e224be08fd1ca435cc9a8d961121869cf6dcd21d2fae7"
SELF="$ROOT_DIR/scripts/training/run_diffusiondrive_grpo_stage35_pilot.sh"
FREEZER="$ROOT_DIR/scripts/evaluation/freeze_grpo_stage35_checkpoints.py"
AUDITOR="$ROOT_DIR/scripts/evaluation/audit_grpo_stage35_checkpoint.py"

[[ "$PHASE" == audit || "$PHASE" == formal || "$PHASE" == chain ]] || {
  echo "phase must be audit, formal, or chain"; exit 2;
}
[[ "$BRANCH" == NCD ]] || {
  echo "Stage35 mainline branch must be NCD"; exit 2;
}
[[ "$HOLDOUT" =~ ^[01]$ ]] || {
  echo "Stage35 pilot holdout must be 0 or 1"; exit 2;
}
[[ "$EXPERIMENT" =~ ^stage35_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid Stage35 experiment name: $EXPERIMENT"; exit 2;
}
[[ "$CUDA_DEVICES" =~ ^[0-7](,[0-7]){7}$ ]] || {
  echo "Stage35 requires exactly eight GPU indices"; exit 2;
}
for path in \
  "$PYTHON_BIN" "$TRAIN_ENTRY" "$PUBLIC" "$SELECTOR" "$CALIBRATION" \
  "$CV_FREEZE" "$DPEL_FREEZE" "$PLAN" "$FREEZER" "$AUDITOR"; do
  [[ -e "$path" ]] || { echo "missing Stage35 input: $path"; exit 2; }
done
[[ "$(sha256sum "$PLAN" | awk '{print $1}')" == "$PLAN_SHA" ]] || {
  echo "Stage35 plan SHA drifted"; exit 2;
}
[[ -d "$TRAIN_CACHE" ]] || {
  echo "missing Stage35 training cache: $TRAIN_CACHE"; exit 2;
}
...
if [[ "$PHASE" == chain ]]; then
  ...
  if [[ "${GRPO_STAGE35_PREFLIGHT_ONLY:-0}" == 1 ]]; then
    "$SELF" audit "$BRANCH" "$HOLDOUT" "$AUDIT_EXPERIMENT" "$CUDA_DEVICES"
    "$SELF" formal "$BRANCH" "$HOLDOUT" "$FORMAL_EXPERIMENT" "$CUDA_DEVICES"
    echo "PASS Stage35 queue-safe chain preflight holdout=$HOLDOUT"
    exit 0
  fi
  "$SELF" audit "$BRANCH" "$HOLDOUT" "$AUDIT_EXPERIMENT" "$CUDA_DEVICES"
  "$SELF" formal "$BRANCH" "$HOLDOUT" "$FORMAL_EXPERIMENT" "$CUDA_DEVICES"
  echo "PASS Stage35 queue-safe chain holdout=$HOLDOUT"
  exit 0
fi

[[ ! -e "$EXP_ROOT/$EXPERIMENT" ]] || {
  echo "refusing to reuse Stage35 experiment directory $EXP_ROOT/$EXPERIMENT"
  exit 2
}
...
for path, expected in (
    ...
    if not path.is_file() or sha(path) != expected:
        raise RuntimeError(f"Stage35 frozen manifest drifted: {path}")
...
if not (
    ...
    raise RuntimeError("Stage35 fold-matched DPEL192 freeze drifted")
initializer = Path(matches[0]["path"])
if not initializer.is_file() or sha(initializer) != allowed[holdout]:
    raise RuntimeError(f"Stage35 DPEL192 initializer drifted: {initializer}")

calibration = json.loads(calibration_path.read_text())
if not calibration.get("passed"):
    raise RuntimeError("Stage35 frozen S-multi calibration did not pass")
print(entry["path"])
print(bucket)
print(initializer)
...
[[ "${#LOCKED[@]}" -eq 6 ]] || {
  echo "failed Stage35 manifest/calibration/DPEL resolution"; exit 2;
}
...
TRAIN_ARGS=(
  ...
  agent.config.stage35_bucket_manifest_path="$BUCKET_MANIFEST"
  agent.config.stage35_plan_sha256="$PLAN_SHA"
  agent.config.stage35_objective_revision=nested_counterfactual_deployment_v1
  agent.config.stage35_active_pool_width=4
  agent.config.stage35_selector_micro_batch_size=16
  agent.config.stage35_frontier_risk_margin=0.1
  agent.config.stage35_counterfactual_weight=0.25
  agent.config.stage35_mature_positive_multiplier=0.25
  agent.config.stage35_advantage_eps=0.001
  agent.config.stage35_delta_scale_floor=0.002
  agent.config.stage35_headroom_low=0.75
  agent.config.stage35_headroom_high=0.9
  agent.config.stage35_rank_weight=0.5
  agent.config.stage35_advantage_clip=2.0
  agent.config.stage35_bc_weight=0.1
  agent.config.stage35_kl_weight=0.1
  agent.config.stage35_safety_kl_weight=0.5
  agent.config.stage35_optimizer_steps_per_epoch=48
  agent.config.stage35_gradient_accumulation=8
  agent.config.stage35_global_bucket_composition=[2,30,8,24]
  agent.config.stage35_signal_nonzero_fraction_min=0.1
  agent.config.stage35_signal_nonowner_scene_fraction_min=0.25
...
if [[ "${GRPO_STAGE35_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  ...
  echo "PASS Stage35 preflight phase=$PHASE holdout=$HOLDOUT"
...
ARTIFACT_DIR="$ROOT_DIR/artifacts/grpo_stage35/pilot/training/NCD/fold${HOLDOUT}/${PHASE}"
FREEZE="$ARTIFACT_DIR/checkpoints.json"
AUDIT="$ARTIFACT_DIR/audit.json"
[[ ! -e "$FREEZE" && ! -e "$AUDIT" ]] || {
  echo "refusing to overwrite Stage35 audit artifacts in $ARTIFACT_DIR"; exit 2;
}
"$PYTHON_BIN" "$FREEZER" \
  --phase "$PHASE" \
  --holdout "$HOLDOUT" \
...
echo "PASS Stage35 training phase=$PHASE branch=$BRANCH holdout=$HOLDOUT"
