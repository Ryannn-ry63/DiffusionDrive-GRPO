...
if [[ $# -ne 2 ]]; then echo "Usage: $0 COV|MCC HOLDOUT(0..3)"; exit 2; fi
BRANCH="$1"; HOLDOUT="$2"
...
[[ "${#GPU_NAMES[@]}" -eq 8 ]] || { echo "Stage30 requires 8 GPUs"; exit 2; }
RUN_TAG="$(date -u +%Y%m%dT%H%M%SZ)"
LOG_DIR="$ROOT_DIR/artifacts/grpo_stage30/h100_logs/${RUN_TAG}_eval_${BRANCH}_fold${HOLDOUT}"
mkdir -p "$LOG_DIR"; cd "$ROOT_DIR"
TOTAL=8; [[ "$BRANCH" == MCC ]] && TOTAL=10
for wave in 0 8; do
  (( wave < TOTAL )) || continue
  pids=(); names=()
  for gpu in 0 1 2 3 4 5 6 7; do
    ...
    bash scripts/evaluation/run_diffusiondrive_grpo_stage30_cv_eval_cell.sh \
...
echo "PASS Stage30 CV evaluation branch=$BRANCH fold=$HOLDOUT"
echo "Logs: $LOG_DIR"
