#!/usr/bin/env python3
"""Apply the frozen Stage30 four-fold OOF MCC selection gate."""
...
def bootstrap_whole_log(values: np.ndarray, logs: list[str]) -> list[float]:
    ...
    if len(grouped) != 606:
        raise RuntimeError(f"Stage30 OOF log count drifted: {len(grouped)}")
...
def load_artifact(
    ...
    if not all(checks):
        raise RuntimeError(f"Stage30 artifact provenance drifted: {path}")
    result = {}
    for record in records:
        ...
        if not (
            ...
            raise RuntimeError(f"Stage30 invalid record: {path}/{token}")
        result[token] = record
    return result
...
def summarize(branch, step, vectors, buckets, folds, noises, logs, component_values):
    ...
    if not all(np.isfinite(value).all() for value in (selected, candidate_mean, oracle, fallback)):
        raise RuntimeError(f"Stage30 non-finite summary vector: {branch}/{step}")
    hard = bucket_ids <= 1
    mature = bucket_ids == 3
    if not hard.any() or not mature.any():
        raise RuntimeError("Stage30 OOF hard/mature bucket is empty")
    ...
    checks = {
        ...
        "all_four_fold_means_strictly_positive": all(x > 0 for x in fold_means.values()),
        "no_fold_below_negative_0.0005": all(x >= -0.0005 for x in fold_means.values()),
        "hard_scene_gain_at_least_0.015": hard_mean >= 0.015,
        ...
        "candidate_mean_delta_strictly_positive": float(candidate_mean.mean()) > 0.0,
        "candidate_max_oracle_delta_at_least_0.0005": float(oracle.mean()) >= 0.0005,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path("artifacts/grpo_stage30/cv/report.json"),
    )
    args = parser.parse_args()
    root = args.root.resolve()
    cv_path = root / "artifacts/grpo_stage30/manifests/cv/freeze.json"
    cv = json.loads(cv_path.read_text(encoding="utf-8"))
    token_buckets = load_stage30_bucket_manifest(cv["bucket_manifest"], require_full=True)
    artifacts = {}
    provenance = {}
    checkpoint_metadata = {}
    ...
    for fold in range(4):
        ...
        if sha256(manifest_path) != entry["holdout_manifest_sha256"]:
            raise RuntimeError(f"Stage30 heldout fold{fold} manifest drifted")
        ...
        for branch in BRANCHES:
            audit_path = root / f"artifacts/grpo_stage30/cv/training/{branch}/fold{fold}/formal/audit.json"
            audit = json.loads(audit_path.read_text(encoding="utf-8"))
            if not (
                audit.get("passed") and audit.get("stage") == 30
                ...
                raise RuntimeError(f"Stage30 formal audit drifted: {branch}/fold{fold}")
    ...
    eligible = [
        summaries[f"MCC{step}"] for step in STEPS
        if summaries[f"MCC{step}"]["eligible"]
    ]
    selected = max(
        eligible,
        key=lambda item: (
            item["whole_log_bootstrap_ci"][0],
            item["mature_scene_mean_delta"],
    ...
    if selected:
        selected_checkpoints = {
            f"fold{fold}": checkpoint_metadata[("MCC", fold)][selected["step"]]
            for fold in range(4)
        }
    result = {
        "schema_version": 1, "stage": 30, "phase": "four_fold_oof",
        ...
        "gate_definition": "stage30_mode_coverage_constrained_oof_v1",
        "bootstrap": {"unit": "whole_log", "seed": BOOTSTRAP_SEED, "samples": BOOTSTRAP_SAMPLES},
        "selected_branch": "MCC" if selected else None,
    ...
    if selected is None:
        raise SystemExit("Stage30 stops: no common MCC step passed the frozen OOF gate")
