...
if [[ $# -ne 2 || ( "$1" != audit && "$1" != formal ) || ( "$2" != H && "$2" != HC ) ]]; then
  echo "Usage: $0 audit|formal H|HC"
  exit 2
fi
PHASE="$1"
BRANCH="$2"
...
ARTIFACT_DIR="$ROOT_DIR/artifacts/grpo_stage29/pilot/training/$BRANCH/$PHASE"
EXPERIMENT="stage29_pilot_${BRANCH}_${PHASE}_seed0"
FREEZE="$ARTIFACT_DIR/checkpoints.json"
AUDIT="$ARTIFACT_DIR/audit.json"
EXPECTED_GPU_SUBSTRING="${STAGE29_EXPECTED_GPU_SUBSTRING:-H100}"

[[ -x "$PYTHON_BIN" ]] || { echo "missing Stage29 Python: $PYTHON_BIN"; exit 2; }
mapfile -t GPU_NAMES < <(nvidia-smi --query-gpu=name --format=csv,noheader)
[[ "${#GPU_NAMES[@]}" -eq 8 ]] || { echo "Stage29 requires exactly 8 visible GPUs"; exit 2; }
for name in "${GPU_NAMES[@]}"; do
  [[ "$name" == *"$EXPECTED_GPU_SUBSTRING"* ]] || {
    echo "Stage29 GPU gate rejected: $name"; exit 2;
  }
done
[[ ! -e "$ARTIFACT_DIR" ]] || {
  echo "refusing to overwrite Stage29 $BRANCH/$PHASE artifacts"; exit 2;
}
if [[ "$PHASE" == formal ]]; then
  "$PYTHON_BIN" - "$ROOT_DIR/artifacts/grpo_stage29/pilot/training/$BRANCH/audit/audit.json" "$BRANCH" <<'PY'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
if not p.is_file():
    raise RuntimeError("Stage29 formal run requires the same branch one-step audit")
d = json.loads(p.read_text())
if not (
    d.get("passed") and d.get("stage") == 29 and d.get("phase") == "audit"
    ...
    raise RuntimeError("Stage29 one-step audit is not a valid passing gate")
PY
fi

cd "$ROOT_DIR"
scripts/training/run_diffusiondrive_grpo_stage29_pilot.sh \
  "$PHASE" "$BRANCH" "$EXPERIMENT" 0,1,2,3,4,5,6,7
shopt -s nullglob
RUN_DIRS=("$NAVSIM_EXP_ROOT/$EXPERIMENT"/*)
[[ "${#RUN_DIRS[@]}" -eq 1 ]] || {
  echo "expected one Stage29 run directory, got ${#RUN_DIRS[@]}"; exit 2;
}
RUN_DIR="${RUN_DIRS[0]}"
"$PYTHON_BIN" scripts/evaluation/freeze_grpo_stage29_pilot_checkpoints.py \
  --phase "$PHASE" --branch "$BRANCH" --run-dir "$RUN_DIR" --output "$FREEZE"
"$PYTHON_BIN" scripts/evaluation/audit_grpo_stage29_pilot_checkpoint.py \
  --phase "$PHASE" --branch "$BRANCH" --base "$PUBLIC" --inputs "$INPUTS" \
  --freeze "$FREEZE" --output "$AUDIT"

echo "PASS Stage29 pilot branch=$BRANCH phase=$PHASE"
echo "Run: $RUN_DIR"
