#!/usr/bin/env python3
"""Paired two-fold Stage35 pilot summary and frozen promotion gate."""
...
def expected_checkpoint(
    ...
    if system == "DPEL192":
        ...
        if len(matches) != 1:
            raise RuntimeError("Stage35 DPEL192 checkpoint freeze drifted")
    ...
    return matches[0]["sha256"], f"stage35_pilot_ncd{step}_fold{fold}"
...
def load_artifact(
    ...
    if not all(checks):
        raise RuntimeError(f"Stage35 artifact provenance drifted: {path}")
    ...
    for record in raw_records:
        ...
        if rewards.shape != (20,) or not np.isfinite(rewards).all():
            raise RuntimeError(f"Stage35 candidate reward shape drifted: {path}")
        ...
        if eligible.shape != (20,) or not 0 <= fallback < 20:
            raise RuntimeError(f"Stage35 eligibility shape drifted: {path}")
    ...
    if not all(np.isfinite(value).all() for value in numeric):
        raise RuntimeError(f"Stage35 non-finite compact artifact: {path}")
    return arrays
...
def main() -> None:
    ...
    if sha256(args.bucket_manifest) != BUCKET_SHA256:
        raise RuntimeError("Stage35 bucket manifest SHA drifted")
    ...
    systems = ("P", "DPEL192") + tuple(f"NCD{step}" for step in STEPS)
    loaded = {}
    checkpoint_shas = {}
    artifact_shas = {}
    for system in systems:
        artifact_root = (
            args.baseline_eval_root
            if system in {"P", "DPEL192"} else args.eval_root
        )
        for fold in (0, 1):
        ...
        for fold in (0, 1):
            for noise in NOISES:
                ...
                if not (
                    ...
                    raise RuntimeError("Stage35 paired token order drifted")
    ...
    result = {
        ...
        "stage35_stop_for_no_generator_ceiling": all(
            comparisons[str(step)]["mean_safe_deployable_oracle_delta"] <= 0
            for step in STEPS
        ),
