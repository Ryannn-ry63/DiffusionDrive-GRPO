...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 audit|formal STAGE24_CHECKPOINT EXPERIMENT_NAME CUDA_DEVICE"
  exit 2
fi
PHASE="$1"; STAGE24_CHECKPOINT="$2"; EXPERIMENT_NAME="$3"; CUDA_DEVICE="$4"
...
BANK_DIR="${STAGE26_BANK_DIR:-$ROOT_DIR/artifacts/grpo_stage26/candidate_bank}"
AUDIT="$ROOT_DIR/artifacts/grpo_stage26/candidate_bank_audit.json"
...
[[ "$EXPERIMENT_NAME" =~ ^stage26_stage25_selector_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid experiment"; exit 2;
}
[[ "$CUDA_DEVICE" =~ ^[0-7]$ ]] || { echo "invalid CUDA device"; exit 2; }
for path in "$STAGE24_CHECKPOINT" "$REFERENCE_CHECKPOINT" "$MANIFEST" "$AUDIT" "${BANKS[@]}"; do
  [[ -f "$path" ]] || { echo "missing locked Stage26 input: $path"; exit 2; }
done
"$PYTHON_BIN" -c '
import hashlib,json,sys
audit=json.load(open(sys.argv[1]))
assert audit["stage"] == 26 and audit["passed"]
assert audit["num_artifacts"] == 15 and audit["count_per_artifact"] == 4075
assert audit["manifest_sha256"] == hashlib.sha256(open(sys.argv[2],"rb").read()).hexdigest()
provenance={item["path"]:item["sha256"] for item in audit["artifacts"]}
for path in sys.argv[3:]:
    assert provenance[path] == hashlib.sha256(open(path,"rb").read()).hexdigest()
' "$AUDIT" "$MANIFEST" "${BANKS[@]}"
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON_BIN" navsim/planning/script/run_training.py \
  ...
  seed=26025 dataloader.params.batch_size=2 dataloader.params.num_workers=4 \
  ...
  agent.config.stage24_selector_dim=128 agent.config.stage24_selector_steps_per_epoch=2038 \
