#!/usr/bin/env python3
"""Freeze exact Stage36 audit/formal checkpoints and run artifacts."""
...
def choose(records: list[dict], step: int) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage36 checkpoint global_step={step} is missing")
    preferred = [
        record
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(
            f"expected one Stage36 Lightning version under "
    ...
    if [record["epoch"] for record in selected] != expected_epochs:
        raise RuntimeError(
            "Stage36 checkpoint epoch/optimizer-step schedule drifted: "
            f"{[(r['epoch'], r['global_step']) for r in selected]}"
        )
    events = sorted(version.glob("events.out.tfevents.*"))
    if len(events) != 1:
        raise RuntimeError("Stage36 expected exactly one TensorBoard event file")
    override_candidates = (
    ...
    if overrides is None:
        raise RuntimeError("Stage36 Hydra overrides are missing")
