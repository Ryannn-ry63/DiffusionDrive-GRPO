...
if [[ $# -ne 2 ]]; then
  echo "Usage: $0 JOB_ID(0..19) GPU(0..7)"
  exit 2
fi
JOB_ID="$1"
GPU="$2"
...
[[ "$JOB_ID" =~ ^([0-9]|1[0-9])$ ]] || { echo "JOB_ID must be 0..19"; exit 2; }
...
if not (p.get('passed') and p.get('stage')==29 and p.get('phase')=='fold4'
        and p.get('all_evaluations_use_safe_multi') is True
        and p.get('historical_control_not_selectable') is True
        and p.get('training_signal_gate_definition')=='stage29_prefold4_amended_dense_scene_v1'
        and p.get('training_signal_gate_amendment_sha256')=='941eef5f370f90ad9e5b9c7ed7a1dbaa12a971f30e79a211c446098f576f3bd9'
        and system in p['systems'] and noise in p['noise_namespaces']):
    raise RuntimeError('Stage29 fold4 frozen grid drifted')
for key in ('manifest','selector_checkpoint','calibration','training_signal_gate_amendment'):
    q=Path(p[key])
    if not q.is_file() or sha(q)!=p[f'{key}_sha256']:
        raise RuntimeError(f'Stage29 fold4 locked input drifted: {key}')
entry=p['systems'][system]
q=Path(entry['path'])
if not q.is_file() or sha(q)!=entry['sha256']:
    raise RuntimeError('Stage29 fold4 generator SHA drifted')
...
[[ "${#LOCKED[@]}" -eq 9 ]] || { echo "failed to resolve Stage29 cell"; exit 2; }
...
if [[ "${GRPO_STAGE29_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  echo "PASS Stage29 fold4 preflight $SYSTEM namespace=$NOISE inputs_sha=$INPUTS_SHA"
  exit 0
fi
...
if bad: raise RuntimeError(f'Stage29 fold4 artifact failed {bad}')
PY
echo "PASS Stage29 fold4 $SYSTEM namespace=$NOISE: $OUTPUT"
