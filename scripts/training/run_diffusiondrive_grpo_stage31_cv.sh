...
if [[ $# -lt 4 || $# -gt 5 ]]; then
  echo "Usage: $0 audit|formal DP|DPF HOLDOUT(0..3) EXPERIMENT [CUDA_DEVICES]"
  exit 2
fi
...
[[ "$EXPERIMENT" =~ ^stage31_cv_(DP|DPF)_fold[0-3]_(audit|formal)_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid Stage31 experiment: $EXPERIMENT"; exit 2;
}
[[ "$CUDA_DEVICES" =~ ^[0-7](,[0-7]){7}$ ]] || { echo "Stage31 requires 8 GPU indices"; exit 2; }
for path in "$PYTHON_BIN" "$TRAIN_ENTRY" "$PUBLIC" "$SELECTOR" "$CALIBRATION" "$CV_FREEZE" "$PLAN"; do
  [[ -e "$path" ]] || { echo "missing Stage31 input: $path"; exit 2; }
done
[[ "$(sha256sum "$PLAN" | awk '{print $1}')" == "$PLAN_SHA" ]] || {
  echo "Stage31 plan SHA drifted"; exit 2;
}
...
if [[ "$BRANCH" == DP ]]; then
  TRAINING_MODE=stage31_public_deployed_pair
else
  TRAINING_MODE=stage31_public_deployed_frontier
fi
...
TRAIN_ARGS=(
  ...
  agent.config.stage31_plan_sha256="$PLAN_SHA"
...
if [[ "${GRPO_STAGE31_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  ...
  echo "PASS Stage31 CV preflight phase=$PHASE branch=$BRANCH holdout=$HOLDOUT"
