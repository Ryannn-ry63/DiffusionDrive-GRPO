...
MANIFEST_DIR="$ROOT_DIR/artifacts/grpo_stage21/manifests"
LOCKED_EPOCH_FILE="${STAGE21_LOCKED_EPOCH_FILE:-}"
...
[[ "$EXPERIMENT_NAME" =~ ^stage21_[A-Za-z0-9_.-]+$ ]] || { echo "experiment must start stage21_"; exit 2; }
...
if [[ "$PHASE" == audit ]]; then
  ...
  [[ "$EPOCHS" == 8 && "$MAX_STEPS" == 0 && "$CUDA_DEVICES" =~ ^[0-7](,[0-7]){7}$ ]] || { echo "development requires 8 GPUs, 8 epochs"; exit 2; }
  ...
  [[ -n "$LOCKED_EPOCH_FILE" && -f "$LOCKED_EPOCH_FILE" ]] || { echo "formal requires STAGE21_LOCKED_EPOCH_FILE"; exit 2; }
...
[[ -f "$MANIFEST" ]] || { echo "Stage21 manifest missing: $MANIFEST"; exit 2; }
...
TRAIN_ARGS=(
  ...
  agent.lr=1e-6 agent.config.grpo_training_mode=diffgrpo_selected_anchor_base_preserve
  ...
  agent.config.generation_policy_algorithm=diffgrpo_selected_anchor_base_preserve
  agent.config.diffgrpo_bc_weight=0.1 agent.config.diffgrpo_bc_max_weight=0.3
  agent.config.diffgrpo_base_margin=0.01 agent.config.diffgrpo_base_scale=0.10
  agent.config.diffgrpo_base_advantage_clip=2.0 agent.config.diffgrpo_safety_regression_tolerance=0.000001
