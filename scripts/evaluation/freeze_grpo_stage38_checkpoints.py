#!/usr/bin/env python3
"""Freeze exact Stage38 ESCR audit/formal checkpoints and run artifacts."""
...
def choose(records: list[dict], step: int) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage38 checkpoint global_step={step} is missing")
    preferred = [
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(
            f"expected one Stage38 Lightning version under "
    ...
    if overrides is None:
        raise RuntimeError("Stage38 Hydra overrides are missing")
    result = {
        "schema_version": 1,
        "stage": 38,
        "phase": args.phase,
