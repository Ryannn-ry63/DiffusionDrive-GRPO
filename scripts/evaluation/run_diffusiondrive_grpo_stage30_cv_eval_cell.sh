...
if [[ $# -ne 4 ]]; then
  echo "Usage: $0 COV|MCC HOLDOUT(0..3) JOB_ID GPU"
  exit 2
fi
...
AUDIT="$ROOT_DIR/artifacts/grpo_stage30/cv/training/$BRANCH/fold$HOLDOUT/formal/audit.json"
OUT_DIR="$ROOT_DIR/artifacts/grpo_stage30/cv/eval/$BRANCH/fold$HOLDOUT"
NOISES=(20261311 20261312)
STEPS=(48 96 144 192)

[[ "$BRANCH" == COV || "$BRANCH" == MCC ]] || { echo "bad branch"; exit 2; }
[[ "$HOLDOUT" =~ ^[0-3]$ ]] || { echo "bad holdout"; exit 2; }
MAX_JOB=7; [[ "$BRANCH" == MCC ]] && MAX_JOB=9
...
for path in "$PUBLIC" "$SELECTOR" "$CALIBRATION" "$CV_FREEZE" "$AUDIT"; do
  [[ -f "$path" ]] || { echo "missing Stage30 eval input: $path"; exit 2; }
done
...
if not (audit.get('passed') and audit.get('stage')==30 and audit.get('phase')=='formal'
        and audit.get('branch')==branch and audit.get('holdout_fold')==fold
        and audit.get('cv_freeze_sha256')==sha(cvp)):
 raise RuntimeError('Stage30 formal audit drifted')
...
if system=='P':
 ...
 if len(matches)!=1: raise RuntimeError('Stage30 checkpoint step missing')
...
if [[ "${GRPO_STAGE30_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  ...
  echo "PASS Stage30 eval preflight $SYSTEM fold=$HOLDOUT ns=$NOISE count=$COUNT"
  exit 0
fi
GRPO_BASE_CHECKPOINT="$PUBLIC" PYTHON_BIN="$PYTHON_BIN" \
  "$ROOT_DIR/scripts/evaluation/run_diffusiondrive_grpo_stage25_selector_eval.sh" \
...
if bad: raise RuntimeError(f'Stage30 eval artifact failed {bad}')
PY
echo "PASS Stage30 eval $SYSTEM fold=$HOLDOUT ns=$NOISE: $OUTPUT"
