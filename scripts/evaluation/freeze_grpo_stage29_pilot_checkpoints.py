#!/usr/bin/env python3
"""Freeze exact Stage29 pilot audit or four-epoch checkpoints."""
...
def choose(candidates: list[dict], step: int, phase: str) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage29 checkpoint global_step={step} is missing")
    pattern = r"grpo-step-\d+\.ckpt" if phase == "audit" else r"grpo-\d{2}-\d+\.ckpt"
    preferred = [
        item for item in matches
        if re.fullmatch(pattern, Path(item["path"]).name)
    ]
    return sorted(preferred or matches, key=lambda item: item["path"])[0]
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(f"expected one Stage29 Lightning version: {versions}")
    version = versions[0]
    candidates = [metadata(path) for path in sorted(
        (version / "checkpoints").glob("*.ckpt")
    )]
    ...
    for item, epoch in zip(selected, expected_epochs):
        if item["epoch"] != epoch:
            raise RuntimeError("Stage29 checkpoint epoch/step schedule drifted")
    events = sorted(version.glob("events.out.tfevents.*"))
    if len(events) != 1:
        raise RuntimeError("Stage29 expected exactly one TensorBoard event")
    override_candidates = (
    ...
    if overrides is None:
        raise RuntimeError("Stage29 Hydra overrides are missing")
    result = {
        "schema_version": 1,
        "stage": 29,
        "phase": args.phase,
    ...
    if args.output.exists():
        raise FileExistsError(f"refusing to overwrite Stage29 freeze: {args.output}")
