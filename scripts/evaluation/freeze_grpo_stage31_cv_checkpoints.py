#!/usr/bin/env python3
"""Freeze exact Stage31 audit or 48/96/144/192-step checkpoints."""
...
def choose(candidates: list[dict], step: int, phase: str) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage31 checkpoint global_step={step} is missing")
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(f"expected one Stage31 Lightning version: {versions}")
    version = versions[0]
    candidates = [
        metadata(path)
    ...
    if [record["epoch"] for record in selected] != expected_epochs:
        raise RuntimeError("Stage31 checkpoint epoch/step schedule drifted")
    events = sorted(version.glob("events.out.tfevents.*"))
    if len(events) != 1:
        raise RuntimeError("Stage31 expected exactly one TensorBoard event")
    override_candidates = (
    ...
    if overrides is None:
        raise RuntimeError("Stage31 Hydra overrides are missing")
    result = {
        "schema_version": 1,
        "stage": 31,
        "phase": args.phase,
