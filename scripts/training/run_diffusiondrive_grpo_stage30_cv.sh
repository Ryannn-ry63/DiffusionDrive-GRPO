...
if [[ $# -lt 4 || $# -gt 5 ]]; then
  echo "Usage: $0 audit|formal COV|MCC HOLDOUT(0..3) EXPERIMENT [CUDA_DEVICES]"
  exit 2
fi
...
[[ "$EXPERIMENT" =~ ^stage30_cv_(COV|MCC)_fold[0-3]_(audit|formal)_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid Stage30 experiment: $EXPERIMENT"; exit 2;
}
[[ "$CUDA_DEVICES" =~ ^[0-7](,[0-7]){7}$ ]] || { echo "Stage30 requires 8 GPU indices"; exit 2; }
for path in "$PYTHON_BIN" "$TRAIN_ENTRY" "$PUBLIC" "$SELECTOR" "$CALIBRATION" "$CV_FREEZE"; do
  [[ -e "$path" ]] || { echo "missing Stage30 input: $path"; exit 2; }
done
...
[[ "${#LOCKED[@]}" -eq 5 ]] || { echo "failed Stage30 input resolution"; exit 2; }
TRAIN_MANIFEST="${LOCKED[0]}"; BUCKET_MANIFEST="${LOCKED[1]}"
MARGIN="${LOCKED[2]}"; RISK="${LOCKED[3]}"; OOD="${LOCKED[4]}"
if [[ "$BRANCH" == COV ]]; then
  TRAINING_MODE=stage30_public_mode_coverage
else
...
TRAIN_ARGS=(
  ...
  agent.config.stage30_bucket_manifest_path="$BUCKET_MANIFEST"
  agent.config.stage30_optimizer_steps_per_epoch=48 agent.config.stage30_gradient_accumulation=8
  agent.config.stage30_global_bucket_composition=[2,30,8,24]
  agent.config.stage30_top_k=5 agent.config.stage30_delta_scale_floor=0.002
  agent.config.stage30_coverage_top_weight=0.5 agent.config.stage30_boundary_top_weight=0.25
  agent.config.stage30_boundary_positive_margin=0.001 agent.config.stage30_boundary_negative_multiplier=2.0
  agent.config.stage30_mature_negative_floor=-0.0002 agent.config.stage30_mature_positive_margin=0.002
  agent.config.stage30_mature_negative_multiplier=4.0 agent.config.stage30_component_tolerance=0.000001
  ...
  agent.config.diffgrpo_group_size=20 agent.config.diffgrpo_base_advantage_clip=2.0
...
if [[ "${GRPO_STAGE30_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  ...
  echo "PASS Stage30 CV preflight phase=$PHASE branch=$BRANCH holdout=$HOLDOUT"
