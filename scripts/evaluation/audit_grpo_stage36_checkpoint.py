#!/usr/bin/env python3
"""Fail-closed Stage36 provenance, optimizer, trace, and gradient audit."""
...
def parse_overrides(path: Path) -> dict[str, str]:
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        ...
        if not line.startswith("- ") or "=" not in line:
            raise RuntimeError(f"invalid Stage36 override: {raw}")
...
def require(actual: dict[str, str], expected: dict[str, str]) -> None:
    for key, wanted in expected.items():
        ...
        if not equivalent:
            raise RuntimeError(
                f"Stage36 override drifted: {key}: "
...
def scalar_values(
    ...
    if len(values) != count or not all(math.isfinite(value) for value in values):
        raise RuntimeError(
            f"Stage36 invalid scalar series {tag}: "
...
def audit_checkpoint(
    ...
    if sha256(path) != record["sha256"]:
        raise RuntimeError("Stage36 checkpoint SHA drifted")
    ...
    if (
        ...
        raise RuntimeError("Stage36 checkpoint metadata drifted")
    initializer_core = {
        name: value
    ...
    if not set(initializer_core).issubset(current):
        raise RuntimeError("Stage36 checkpoint lost an initializer tensor")

    allowed, forbidden, by_layer = [], [], {0: 0, 1: 0}
    for name, before in initializer_core.items():
        after = current[name]
        if not torch.isfinite(after).all():
            raise RuntimeError(f"Stage36 non-finite tensor: {name}")
    ...
    if forbidden or len(allowed) != 64 or by_layer != {0: 32, 1: 32}:
        raise RuntimeError(
            f"Stage36 decoder boundary drifted: allowed={len(allowed)} "
    ...
    for name, before in public.items():
        ...
        if reference_name not in current or not torch.equal(
            ...
            raise RuntimeError(f"Stage36 frozen reference drifted: {reference_name}")
    ...
    if len(old_names) != len(decoder_names):
        raise RuntimeError("Stage36 old-policy snapshot is incomplete")
    if not all(torch.isfinite(current[name]).all() for name in old_names):
        raise RuntimeError("Stage36 old-policy snapshot is non-finite")
    # The generic PPO hook synchronizes old_policy every 32 optimizer steps;
    # Stage36 does not consume it, so equality to the initial DPEL192 state is
    ...
    if groups != [(0.1, 1e-7, 32, 0.0), (1.0, 1e-6, 32, 0.0)]:
        raise RuntimeError(f"Stage36 optimizer groups drifted: {groups}")
    for item in optimizers[0]["state"].values():
        for value in item.values():
            if torch.is_tensor(value) and not torch.isfinite(value).all():
                raise RuntimeError("Stage36 optimizer has non-finite state")
    return {
        **record,
...
def main() -> None:
    ...
    if sha256(args.initializer) not in STAGE36_ALLOWED_INITIAL_SHA256:
        raise RuntimeError("Stage36 initializer is not a frozen DPEL192 checkpoint")
    if sha256(args.base) != STAGE30_PUBLIC_SHA256:
        raise RuntimeError("Stage36 public-88.1 checkpoint drifted")
    if sha256(args.selector) != STAGE30_SELECTOR_SHA256:
        raise RuntimeError("Stage36 frozen S-multi selector drifted")
    if sha256(args.calibration) != STAGE30_CALIBRATION_SHA256:
        raise RuntimeError("Stage36 selector calibration drifted")
    if sha256(args.plan) != STAGE36_PLAN_SHA256:
        raise RuntimeError("Stage36 plan SHA drifted")
    ...
    for path, expected in (
        ...
        if not path.is_file() or sha256(path) != expected:
            raise RuntimeError(f"Stage36 frozen input drifted: {path}")

    freeze = json.loads(args.freeze.read_text(encoding="utf-8"))
    expected_steps = [1] if args.phase == "audit" else [48, 96, 144, 192]
    if not (
        freeze.get("passed")
        and freeze.get("stage") == 36
        ...
        and freeze.get("objective_revision") == STAGE36_OBJECTIVE_REVISION
        and freeze.get("plan_sha256") == STAGE36_PLAN_SHA256
    ):
        raise RuntimeError("Stage36 checkpoint freeze semantics drifted")
    ...
    if (
        ...
        raise RuntimeError("Stage36 run artifact SHA drifted")
    ...
    required = {
        ...
        "agent.config.stage36_bucket_manifest_path": cv["bucket_manifest"],
        "agent.config.stage36_plan_sha256": STAGE36_PLAN_SHA256,
        "agent.config.stage36_objective_revision": STAGE36_OBJECTIVE_REVISION,
    ...
    for name, value in STAGE36_ROUTE_EXPECTED.items():
        rendered = (
    ...
    if args.phase == "audit":
        ...
        raise RuntimeError("Stage36 formal run unexpectedly limits max_steps")
    ...
    metric_names = (
        ...
        "sampled_chain_count",
        "current_unique_replay_count",
        "public_unique_replay_count",
        "current_physical_replay_count",
        "public_physical_replay_count",
        "reward_dependent_replay",
        "replay_deduplicated",
        "counterfactual_selector_count",
    )
    finite_tags = common_tags | {
        f"train/stage36_{name}_step" for name in metric_names
    }
    active_tags = {
    ...
    if missing:
        raise RuntimeError(f"Stage36 TensorBoard tags missing: {missing}")
    values = {
    ...
    if any(min(values[tag]) <= 0 for tag in active_tags):
        raise RuntimeError("Stage36 permitted decoder gradient is inactive")
    if any(any(value != 0 for value in values[tag]) for tag in frozen_tags):
        raise RuntimeError("Stage36 frozen-module gradient is nonzero")
    exact_tags = {
        "train/stage36_candidate_level_trace_step": 1.0,
        "train/stage36_same_anchor_only_step": 1.0,
        "train/stage36_sampled_chain_count_step": 320.0,
        "train/stage36_reward_dependent_replay_step": 1.0,
        "train/stage36_frontier_exact_width_step": 1.0,
        "train/stage36_tail_exact_width_step": 1.0,
        "train/stage36_retention_target_consistency_step": 1.0,
        "train/stage36_tail_target_consistency_step": 1.0,
        "train/stage36_replay_deduplicated_step": 1.0,
        "train/stage36_counterfactual_selector_count_step": 224.0,
    }
    for tag, wanted in exact_tags.items():
        if any(value != wanted for value in values[tag]):
            raise RuntimeError(f"Stage36 trace invariant drifted: {tag}")

    retention = values["train/stage36_frontier_retention_active_fraction_step"]
    tail = values["train/stage36_tail_expansion_active_fraction_step"]
    retention_mean = sum(retention) / len(retention)
    tail_mean = sum(tail) / len(tail)
    signal_health = {
        "frontier_retention_active_fraction_mean": retention_mean,
        "tail_expansion_active_fraction_mean": tail_mean,
        "frontier_retention_active_fraction_min": STAGE36_ROUTE_EXPECTED[
            "stage36_signal_retention_fraction_min"
        ],
        "tail_expansion_active_fraction_min": STAGE36_ROUTE_EXPECTED[
            "stage36_signal_tail_fraction_min"
        ],
    }
    signal_health["passed"] = (
        retention_mean >= signal_health["frontier_retention_active_fraction_min"]
        and tail_mean >= signal_health["tail_expansion_active_fraction_min"]
    )
    # Counts are logged with ``sync_dist=True`` and therefore are rank means,
    # not integer per-rank totals.  The scientific contract is that each
    # reward-dependent pool is non-empty, deduplicated, and never evaluates
    # more than the original 8 x 20 bank.  A fixed integer lower bound would
    # incorrectly reject valid sparse public-retention pools.
    replay_pairs = (
        ("train/stage36_current_unique_replay_count_step",
         "train/stage36_current_physical_replay_count_step"),
        ("train/stage36_public_unique_replay_count_step",
         "train/stage36_public_physical_replay_count_step"),
    )
    for unique_tag, physical_tag in replay_pairs:
        for unique, physical in zip(values[unique_tag], values[physical_tag]):
            if not (0 < unique <= physical <= 160):
                raise RuntimeError(
                    f"Stage36 sparse replay boundary drifted: "
                    f"{unique_tag}={unique}, {physical_tag}={physical}"
                )
    if args.phase == "audit" and not signal_health["passed"]:
        raise RuntimeError(
            f"Stage36 reference-tail signal is unhealthy: {signal_health}"
        )
    ...
    result = {
        ...
        "passed": True,
        "training_mode": "stage36_reference_gated_tail_ncd_grpo",
        ...
        "reference_tail_signal_health": signal_health,
