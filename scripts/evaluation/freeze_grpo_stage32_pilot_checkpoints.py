#!/usr/bin/env python3
"""Freeze exact Stage32 pilot checkpoints for audit or formal evaluation."""
...
def choose(candidates: list[dict], step: int, phase: str) -> dict:
    ...
    if not matches:
        raise RuntimeError(f"Stage32 checkpoint global_step={step} is missing")
...
def main() -> None:
    ...
    if len(versions) != 1:
        raise RuntimeError(f"expected one Stage32 Lightning version: {versions}")
    version = versions[0]
    candidates = [
        metadata(path)
    ...
    expected_epochs = [0] if args.phase == "audit" else [3, 7, 11]
    ...
    if [record["epoch"] for record in selected] != expected_epochs:
        raise RuntimeError("Stage32 checkpoint epoch/step schedule drifted")
    events = sorted(version.glob("events.out.tfevents.*"))
    if len(events) != 1:
        raise RuntimeError("Stage32 expected exactly one TensorBoard event")
    override_candidates = (
    ...
    if overrides is None:
        raise RuntimeError("Stage32 Hydra overrides are missing")
    result = {
        "schema_version": 1,
        "stage": 32,
        "phase": args.phase,
        ...
        "objective_revision": (
            "mean_normalized_bc_kl_v2" if args.branch == "SCF" else "stage31_control"
        ),
    }
