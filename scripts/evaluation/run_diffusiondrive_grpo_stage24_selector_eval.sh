...
SELECTOR_ARGS=(
  --selector-logits-source trajectory_safety_value_v2
  --stage24-selector-checkpoint "$SELECTOR"
)
if [[ "$MODE" == collect ]]; then
  [[ "$CALIBRATION" == - ]] || { echo "collect mode requires calibration '-'"; exit 2; }
  SELECTOR_ARGS+=(--stage24-collect-calibration)
else
  [[ -f "$CALIBRATION" ]] || { echo "missing Stage24 calibration"; exit 2; }
...
' "$CALIBRATION")
  SELECTOR_ARGS+=(
    --stage24-selector-calibration "$CALIBRATION"
    --stage24-selector-residual-margin "$MARGIN"
    --stage24-selector-risk-threshold "$RISK"
