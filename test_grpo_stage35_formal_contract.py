...
def test_stage35_formal_contract_accepts_only_frozen_ncd_route():
    config = _config()
    assert STAGE35_GRPO_MODES <= FORMAL_GRPO_MODES
    validate_formal_grpo_config(config)
    config.stage35_counterfactual_weight = 0.5
    with pytest.raises(ValueError, match="stage35_counterfactual_weight"):
        validate_formal_grpo_config(config)


def test_stage35_requires_own_bucket_and_frozen_selector():
    config = _config()
    config.stage35_bucket_manifest_path = ""
...
def test_stage35_resolves_own_bucket_and_two_fold_initializers():
    config = _config()
    ...
    assert stage == "Stage35"
    assert path == "buckets.json"
    assert STAGE35_ALLOWED_INITIAL_SHA256 == {
