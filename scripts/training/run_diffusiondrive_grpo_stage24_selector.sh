...
[[ "$EXPERIMENT_NAME" =~ ^stage24_selector_[A-Za-z0-9_.-]+$ ]] || { echo "invalid experiment"; exit 2; }
[[ "$CUDA_DEVICE" =~ ^[0-7]$ ]] || { echo "invalid CUDA device"; exit 2; }
for path in "$BASE_CHECKPOINT" "$MANIFEST" "${BANKS[@]}"; do
  [[ -f "$path" ]] || { echo "missing locked Stage24 input: $path"; exit 2; }
done
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICE" "$PYTHON_BIN" navsim/planning/script/run_training.py \
  ...
  seed=24024 dataloader.params.batch_size=2 dataloader.params.num_workers=4 \
