...
REPORT="$ROOT_DIR/artifacts/grpo_stage28/pilot/fold4/report.json"
RUN_TAG="$(date -u +%Y%m%dT%H%M%SZ)"
LOG_DIR="$ROOT_DIR/artifacts/grpo_stage28/h100_logs/${RUN_TAG}_pilot_fold4"
[[ -f "$INPUTS" ]] || { echo "missing Stage28 fold4 inputs: $INPUTS"; exit 2; }
[[ ! -e "$REPORT" ]] || { echo "refusing to overwrite Stage28 fold4 report"; exit 2; }
if [[ -e "$OUT_DIR" ]]; then
  [[ -d "$OUT_DIR" && -z "$(find "$OUT_DIR" -mindepth 1 -maxdepth 1 -print -quit)" ]] || {
    echo "refusing to overwrite Stage28 fold4 results"; exit 2;
  }
fi
mapfile -t GPU_NAMES < <(nvidia-smi --query-gpu=name --format=csv,noheader)
[[ "${#GPU_NAMES[@]}" -eq 8 ]] || { echo "Stage28 fold4 requires exactly 8 visible GPUs"; exit 2; }
mkdir -p "$LOG_DIR"
cd "$ROOT_DIR"
SYSTEMS=(P A1 A2 A3 A4 B1 B2 B3 B4 C1 C2 C3 C4)
NOISES=(20261111 20261112)

for wave_start in 0 8 16 24; do
  pids=(); names=()
  for offset in 0 1 2 3 4 5 6 7; do
    ...
    names+=("$name")
    bash scripts/evaluation/run_diffusiondrive_grpo_stage28_fold4_cell.sh \
  ...
  for index in "${!pids[@]}"; do
    if wait "${pids[$index]}"; then
      echo "PASS Stage28 fold4 cell ${names[$index]}"
    else
      status=$?; echo "FAIL Stage28 fold4 cell ${names[$index]} exit=$status"; failed=1
    fi
  done
  (( failed == 0 )) || { echo "inspect Stage28 logs: $LOG_DIR"; exit 1; }
done
...
"$PYTHON_BIN" scripts/evaluation/check_grpo_stage28_fold4.py \
  "${GATE_ARGS[@]}" --output "$REPORT"
echo "PASS Stage28 pilot fold4 gate"
echo "Report: $REPORT"
echo "Logs: $LOG_DIR"
