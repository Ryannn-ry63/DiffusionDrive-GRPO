...
if [[ $# -lt 3 || $# -gt 4 ]]; then
  echo "Usage: $0 audit|formal A|B|C EXPERIMENT [CUDA_DEVICES=0,1,2,3,4,5,6,7]"
  exit 2
fi
...
[[ "$PHASE" == audit || "$PHASE" == formal ]] || {
  echo "Stage28 pilot phase must be audit or formal"
  exit 2
}
[[ "$BRANCH" =~ ^[ABC]$ ]] || {
  echo "Stage28 pilot branch must be A, B, or C"
  exit 2
}
[[ "$EXPERIMENT" =~ ^stage28_pilot_[ABC]_(audit|formal)_[A-Za-z0-9_.-]+$ ]] || {
  echo "invalid Stage28 experiment name: $EXPERIMENT"
  exit 2
}
[[ "$CUDA_DEVICES" =~ ^[0-7](,[0-7]){7}$ ]] || {
  echo "Stage28 pilot requires eight distinct GPU indices"
  exit 2
}
...
for path in "$PYTHON_BIN" "$TRAIN_ENTRY" "$INPUTS"; do
  [[ -e "$path" ]] || { echo "missing Stage28 pilot input: $path"; exit 2; }
done
[[ -d "$TRAIN_CACHE" ]] || { echo "missing training cache: $TRAIN_CACHE"; exit 2; }
[[ ! -e "$EXP_ROOT/$EXPERIMENT" ]] || {
  echo "refusing to reuse Stage28 experiment: $EXP_ROOT/$EXPERIMENT"
  exit 2
}
...
if not p.get("passed") or p.get("stage") != 28 or branch not in p["branches"]:
    raise RuntimeError("Stage28 pilot frozen branch drifted")
...
for key in ("public_checkpoint", "manifest", selector_key, calibration_key):
    value = Path(p[key])
    if not value.is_file() or sha(value) != p[f"{key}_sha256"]:
        raise RuntimeError(f"Stage28 pilot locked input drifted: {key}")
calibration = json.loads(Path(p[calibration_key]).read_text(encoding="utf-8"))
print(p["public_checkpoint"])
print(p["manifest"])
print(entry["training_mode"])
print(role)
print(p[selector_key])
print(p[calibration_key])
print(calibration["residual_margin"])
print(calibration["risk_threshold"])
print(calibration["ood_threshold"])
print(p["exploration_authorization"] if role == "explore" else "")
PY
)
[[ "${#LOCKED[@]}" -eq 10 ]] || { echo "failed to resolve Stage28 inputs"; exit 2; }
PUBLIC="${LOCKED[0]}"
MANIFEST="${LOCKED[1]}"
TRAINING_MODE="${LOCKED[2]}"
SELECTOR_ROLE="${LOCKED[3]}"
SELECTOR="${LOCKED[4]}"
CALIBRATION="${LOCKED[5]}"
MARGIN="${LOCKED[6]}"
RISK="${LOCKED[7]}"
OOD="${LOCKED[8]}"
AUTHORIZATION="${LOCKED[9]}"
...
if [[ "$BRANCH" == B || "$BRANCH" == C ]]; then
  TRAIN_ARGS+=(
    agent.config.diffgrpo_base_scale=0.1
    ...
    agent.config.stage28_training_selector_role="$SELECTOR_ROLE"
  )
fi
if [[ "$BRANCH" == C ]]; then
  TRAIN_ARGS+=(agent.config.stage28_exploration_authorization_path="$AUTHORIZATION")
fi

if [[ "${GRPO_STAGE28_PREFLIGHT_ONLY:-0}" == 1 ]]; then
  echo "PASS Stage28 pilot preflight phase=$PHASE branch=$BRANCH"
  echo "Mode: $TRAINING_MODE"
  echo "Selector role: $SELECTOR_ROLE"
